...
export async function getTransactionReceipt(query) {
  ...
  for(const ethHash of allEthHashInRange) {
    ...
    if(allReceipts.result == null) {
      window.alert("Bother you to please come back? I must run to Crystalvale to replenish my quill!")
    }
  ...
  allQuestCompleted.forEach((questComplete) => {
    ...
    try {
      ...
      const key = questComplete.blockHash + questComplete.logIndex
      ...
      allQuestCompletedDecoded.push({ key, id, questContract, questId, player, heroId, startTime, startBlock, completeAtTime })
    }
    catch(e) {
      console.log(e)
    }
  ...
  allQuestRewards.forEach((questReward) => {
    ...
    try {
      ...
      const key = questReward.blockHash + questReward.logIndex
      const questId = eventLog.questId.toNumber()
      const player = eventLog.player
      const heroId = eventLog.heroId.toNumber()
      ...
      if(rewardItem !== "Nothing") {
        allQuestRewardsDecoded.push({ key, questId, player, heroId, rewardItem, itemQuantity })
      }
    }
    catch(e) {
      console.log(e)
